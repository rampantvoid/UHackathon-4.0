...
class TrafficSignalController:
    ...
    def print_signal_durations(self, intersection_name):
        ...
        for lane, data in intersection.items():
            ...
            queue.append((lane,green_duration))
            # print(f"{lane}: Green - {green_duration} seconds, Yellow - {yellow_duration} seconds, Current Lane: {current_lane}")
        def Sort_Tuple(tup):
            tup.sort(key = lambda x: x[1])
            return tup
        
        newqueue = Sort_Tuple(queue)
        newqueue.reverse()
        for lane, green_duration in newqueue:
            print(f"{lane}: Green - {green_duration} seconds, Yellow - 5 seconds")
            df = pd.DataFrame([lane,green_duration,datetime.now()])
...
def detectTraffic(videoarea):
    ...
    for video,area in videoarea:
        ...
        cv2.imwrite('results/result.jpg',frame)
